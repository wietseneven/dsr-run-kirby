	...
	L.Control.BigImage = L.Control.extend({
			...
			_getMarkerLayer: function (layer, resolve) {
					...
					if (!self._pointPositionIsNotCorrect(pixelPoint) && layer._icon.src) {
						console.log(layer._icon.offsetLeft, pixelPoint.x + layer._icon.offsetLeft, pixelPoint);
							let image = new Image();
							image.crossOrigin = 'Anonymous';
							...
							image.onload = function () {
									self.markers[layer._leaflet_id] = {
										img: image,
										x: pixelPoint.x + layer._icon.offsetLeft,
										y: pixelPoint.y + layer._icon.offsetTop,
										width: layer._icon.width,
										height: layer._icon.height
									};
									resolve();
							};
							return;
